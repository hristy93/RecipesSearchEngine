...
RULES_FILE = "{base_dir}/bulgarian_stemmer/stem_rules_context_2_UTF-8.txt".format(base_dir=BASE_DIR)
MIN_RULE_FREQ = 2
MIN_WORD_LEN = 3
...
def fetchTheRules(RULES_FILE, MIN_RULE_FREQ):
    ...
    for rule in codecs.open(RULES_FILE, 'r', 'utf-8').readlines():
        ...
        if rule_parts != None:
            if int(rule_parts.group(3)) > MIN_RULE_FREQ:
...
try:
    StemmingRules = cPickle.load(open('rules/StemmingRules-MinFreq-'+str(MIN_RULE_FREQ)+'.pickle', 'rb'))
except:
